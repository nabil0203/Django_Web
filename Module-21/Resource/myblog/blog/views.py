...
def post_list(request):
    ...
    posts = Post.objects.all() # all post ke anlam
    ...
    paginator = Paginator(posts, 2) # per page a 2 ta kore post dekhabe
    # 100 ta post ache, per page e 10 ta post hole, total 10 ta page lagbe
...
def like_post(request, id):
    ...
    if post.liked_users.filter(id=request.user.id): # user like diche
        post.liked_users.remove(request.user)
    else: # user like dey nai
